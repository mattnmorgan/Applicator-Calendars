...
import { Tooltip, ButtonIcon, Button, DynamicInput, Modal } from "@applicator/sdk/components";
...
function CategoryModal({
  ...
  const footer = (
    <>
      ...
      <Button variant="primary" onClick={handleSave} disabled={saving}>{existing ? "Save" : "Create"}</Button>
    </>
  );
  ...
  return (
    ...
    >
      <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
        <DynamicInput
          input={{ id: "name", label: "Name", type: "text", required: true, placeholder: "Category name" }}
          value={name}
...
function DeleteCategoryModal({
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 8 }}>Delete "{category.name}"?</div>
        <div style={{ fontSize: 13, opacity: 0.7, marginBottom: 20 }}>
          What should happen to events in this category?
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <Button variant="secondary" fullWidth onClick={() => onConfirm("migrate")}>
            Remove category only (keep events)
          </Button>
          <Button variant="danger" fullWidth onClick={() => onConfirm("delete")}>
...
export default function CalendarSidebar({
  ...
  const [editingCategory, setEditingCategory] = React.useState<CategoryData | undefined>(undefined);
  ...
  const sortedSubs = [...subscriptions].sort((a, b) => a.name.localeCompare(b.name));
        ...
        {!collapsed && allItems.length > 0 && (
          <>
            ...
            >
              ...
              {canEdit && (
                <Tooltip text="New category" placement="right">
                  <ButtonIcon
                    name="plus"
                    label="New category"
                    size="sm"
            ...
            {allItems.map((item) => {
              ...
              const isChecked = item.kind === "category"
              ...
              return (
                ...
                >
                  <input
                    type="checkbox"
                    checked={isChecked}
                    onChange={() => item.kind === "category" ? onToggleCategory(id) : onToggleSub(id)}
                  ...
                  >
                    {item.data.name}
                    {item.kind === "sub" && (
                      <span style={{ fontSize: 10, opacity: 0.5, marginLeft: 4 }}>ICS</span>
                    )}
                  </span>

                  {/* Hover-only action buttons */}
                  <div style={{ display: "flex", gap: 1, opacity: isHovered ? 1 : 0, transition: "opacity 0.1s", flexShrink: 0 }}>
                    {item.kind === "category" && canEdit && (
                      <ButtonIcon
                        name="edit"
                        label="Edit category"
                        size="sm"
                        onClick={() => { setEditingCategory(item.data as CategoryData); setShowCategoryModal(true); }}
                      />
                    )}
                    {item.kind === "category" && canEdit && (
                      <ButtonIcon
                        ...
                        size="sm"
                        subvariant="danger"
                        onClick={() => setDeletingCategory(item.data as CategoryData)}
                      />
                    )}
                    {item.kind === "sub" && canEdit && (
                      <Tooltip text="Unsubscribe" placement="left">
                        <ButtonIcon
                          name="close"
                          label="Unsubscribe"
                          size="sm"
                          subvariant="danger"
                          onClick={() => handleUnsubscribe(item.data as IcsSubscriptionData)}
                        />
                      </Tooltip>
                    )}
                  </div>
