...
import { CalendarRecord, EventRecord, EventData, TodoRecord, TodoData, TodoStatus } from "@/src/types";
...
export async function GET(
  ...
  const [evResult, todoResult] = await Promise.all([
    eventsRm.readRecords({ fields: { calendarId: cal.id }, limit: 5000 }),
    todosRm.readRecords({ fields: { calendarId: cal.id }, limit: 5000 }),
  ]);

  const events = evResult.records.map(parseEventData);
  const todos = todoResult.records.map(parseTodoData);

  const icsContent = generateICS(cal.data.name, events, todos);
