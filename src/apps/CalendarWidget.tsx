...
export default function CalendarWidget({ context: _context, settings, maxHeight }: Props) {
  ...
  const containerHeight = maxHeight ? `calc(${maxHeight} - 48px)` : "100%";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: containerHeight, overflow: "hidden", position: "relative", color: "#e2e8f0" }}>
      <CalendarView
        viewMode={viewMode}
